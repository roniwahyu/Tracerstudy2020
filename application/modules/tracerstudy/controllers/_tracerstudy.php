...
class Tracerstudy extends MX_Controller{
	function __construct(){
        ...
        $this->load->model('jawab_mhs/jawab_mhs_model','jawabdb',TRUE);
    ...
    function getmhs($username){
        $this->db->select('a.*')->from('mhsts2019 a')->join('users b','a.nim=b.nim')->where('b.username',$username);
        $result=$this->db->get();
        $this->_result($result);
    ...
    function mhs(){
        ...
        // if(isset($nim) || !empty($nim) || $nim>0){
            $this->template->set_layout('narrow');
            $this->template->add_js('var baseurl="'.base_url('admin/jawab_mhs').'/";','embed');
            $this->template->add_js('var jawaburl="'.base_url().'jawab_mhs/";','embed');  
        ...
        // $this->template->set_themes(THEMES);
         ...
         // print_r($user->nim);
         print_r($mhs);
            $this->template->load_view('tracer',array('user'=>$user,'data'=>$this->getmhs($user->username)));
        // }
