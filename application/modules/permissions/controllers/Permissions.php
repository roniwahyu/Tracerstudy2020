...
class Permissions extends Admin_Controller
{
    ...
    */
    ...
    public function getdtables(){
        ...
        $this->dtables->select('id,name,description,create,read,update,delete,setup,isactive')->from('permissions');
        $this->dtables->edit_column('isactive','$1','isactivebutton(isactive,id)');
        $show=base_url('permissions/show/');
        $edit=base_url('permissions/edit/');
        $target="#kt_modal_4";

        $this->dtables->add_column('aksi','<div class="btn-group">$1 $2 $3</div>','viewbtn(kode,'.$show.','.$target.'),editbtn(kode,'.$edit.','.$target.'),delbtn(kode)');

        echo $this->dtables->generate();

        // echo ($this->dtables->select('id,name,description,isactive,create,read,update,delete,setup,report,print,export,import,userid,created,modified,')->from('permissions')->generate());
    }
